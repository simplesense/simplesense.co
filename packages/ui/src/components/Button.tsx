...
/**
 * SimpleSense Button (ported from the design system). Primary/clay carry the signature
 * inner top glint; press states shift color, never shrink.
 */
export function Button({
  ...
  const sizes: Record<ButtonSize, { padding: string; height: number; fontSize: number }> = {
  ...
  const base: CSSProperties = {
    ...
    transition:
      'background var(--dur-fast) var(--ease-in-out), color var(--dur-fast), border-color var(--dur-fast)',
    whiteSpace: 'nowrap',
    userSelect: 'none',
    overflow: 'hidden',
  }
  ...
  const variants: Record<ButtonVariant, CSSProperties> = {
    ...
    clay: {
      background: active || hover ? 'var(--ss-clay-600)' : 'var(--ss-clay-500)',
      color: 'var(--text-onbrand)',
  ...
  return (
    ...
    >
      {glint}
      {icon ? (
        <i className={`bi bi-${icon}`} aria-hidden="true" style={{ fontSize: '1.05em' }} />
      ) : null}
      <span style={{ position: 'relative' }}>{children}</span>
      {iconRight ? (
        <i className={`bi bi-${iconRight}`} aria-hidden="true" style={{ fontSize: '1.05em' }} />
      ) : null}
    </button>
  )
}
